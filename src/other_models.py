...
def polyadic_closure_1(
    ...
    seed: int | None = None, ):
    """
    First version of the polyadic closure model. In this version, we sample from a neighborhood, and if the neighborhood is not big enough we fill the edge with random nodes. There is an option for dynamic target set selection (see parameters). 
    Parameters
    ----------
    n : int
        Final number of nodes |V| in the returned hypergraph.
...
def node_addition_preferential_attachment(
    n: int,
    M: dict,
    k0 : int = 1,
    n0: int =20,
    ...
    for i in range(n0 , n ):
        ...
        while Di:
            ...
            S = _random_subset(duplicated_nodes, d - 1, rng)
            e = {i} | S
